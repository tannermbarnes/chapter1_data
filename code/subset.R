...
sites_with_sufficient_data %>% filter(year > 2013) %>% 
...
model_this_data <- species_recovery_with_beta %>% 
pivot_wider(names_from = year, 
            values_from = total_count) %>% 
select(site, slope) %>% 
left_join(data_wide2, by = "site")

# Model this data is the data with the slope estimate per site with 
# all the correct metadata to model with for the data since minimum count
# so for the mines that are recovering
